...
class Trainerhelp:
    ...
    def load_data(self):
        ...
        if self.task=='slot':
            ...
            return train_sent,test_sent,train_Y,test_Y
        if self.task=='both':
            train_IN_Y=EmbeddingDictionary.read_file(self,os.path.abspath('./datasets/'+self.dataset+'/'+self.language+'/intent_train.txt'))
            test_IN_Y=EmbeddingDictionary.read_file(self,os.path.abspath('./datasets/'+self.dataset+'/'+self.language+'/intent_test.txt'))
            train_SO_Y=EmbeddingDictionary.read_file(self,os.path.abspath('./datasets/'+self.dataset+'/'+self.language+'/slot_train.txt'))
            test_SO_Y=EmbeddingDictionary.read_file(self,os.path.abspath('./datasets/'+self.dataset+'/'+self.language+'/slot_test.txt'))
            return train_sent,test_sent,train_IN_Y,test_IN_Y, train_SO_Y,test_SO_Y
    ...
    def load_label_dict(self):
        ...
        if self.task=='slot':
            slot2id=np.load(os.path.abspath('./datasets/'+self.dataset+'/'+self.language+'/slot2id.npy')).item()
            id2slot=np.load(os.path.abspath('./datasets/'+self.dataset+'/'+self.language+'/id2slot.npy')).item()
